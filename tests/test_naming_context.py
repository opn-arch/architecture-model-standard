...
def test_format_naming_context_compact():
    ...
    result = DecomposeResult(
        ...
        sub_components=[
            SubComponent(id="COMP-S6-1", name="temp", files=["mqtt/client.py", "mqtt/transport.py"],
                        classes=["MQTT", "MqttClientSetup", "AsyncTransport"], functions=["connect", "disconnect"], line_count=800),
            SubComponent(id="COMP-S6-2", name="temp", files=["mqtt/discovery.py", "mqtt/models.py"],
                        classes=["MqttDiscovery", "DiscoveryPayload"], functions=["async_start", "process_message"], line_count=600),
        ],
        internal_relationships=[
            InternalRelationship(from_id="COMP-S6-2", to_id="COMP-S6-1", edge_count=5),
        ],
    )

    context = format_naming_context(result)
    assert "COMP-S6-1" in context
...
def test_format_naming_context_empty():
    ...
    result = DecomposeResult(block_id="S1", block_name="Core")
    context = format_naming_context(result)
