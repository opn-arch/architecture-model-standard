"""Tests for auto_source_blocks generation."""
import pytest
from architecture_model.manifest.grouping import auto_source_blocks, ModuleGroup


class TestAutoFblocks:
    def test_large_groups_become_source_blocks(self):
        groups = [
        ...
        result = auto_source_blocks(groups, threshold=3)
        assert "S1" in result
        assert "S2" in result
        assert result["S1"]["name"] == "Auth"
        assert result["S2"]["name"] == "API"
        assert len(result["S2"]["files"]) == 4
    ...
    def test_small_groups_merged_to_shared(self):
        ...
        result = auto_source_blocks(groups, threshold=3)
        assert "S1" in result  # Auth
        assert "S0" in result  # Shared (Utils + Config merged)
        assert "utils.py" in result["S0"]["files"]
        assert "config.py" in result["S0"]["files"]
        assert len(result["S0"]["files"]) == 3
    ...
    def test_common_dir_detected(self):
        ...
        result = auto_source_blocks(groups)
        assert result["S1"]["dirs"] == ["src/backend"]
    ...
    def test_no_common_dir_when_mixed(self):
        ...
        result = auto_source_blocks(groups)
        assert result["S1"]["dirs"] == [] or result["S1"]["dirs"] == [""]

    def test_empty_groups(self):
        result = auto_source_blocks([])
        assert result == {}
    ...
    def test_all_small_groups(self):
        ...
        assert "S1" in result
        assert "S2" in result
        assert result["S1"]["files"] == ["a.py"]
        assert result["S2"]["files"] == ["b.py"]
    ...
    def test_flat_repo_fallback_promotes_groups(self):
        ...
        # Should NOT collapse everything to S0
    ...
    def test_flat_repo_fallback_not_triggered_when_source_blocks_exist(self):
        ...
        # "core" meets threshold -> S1, "small" -> S0
        assert "S1" in result
        assert "S0" in result
    ...
    def test_threshold_customizable(self):
        ...
        result = auto_source_blocks(groups, threshold=2)
        assert "S1" in result
        # With threshold=3, it goes to Shared
        result2 = auto_source_blocks(groups, threshold=3)
        assert "S0" in result2
