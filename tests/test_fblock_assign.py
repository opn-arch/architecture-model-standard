"""Tests for deterministic tie-breaking in auto_assign_f_blocks."""

from architecture_model.core.fblock_assign import auto_assign_f_blocks
...
def test_deterministic_tiebreak_alphabetical():
    ...
    for _ in range(5):
        ...
        assigned = auto_assign_f_blocks(model)
        fb_map = {c.id: c.f_block for c in assigned.entities.components}
...
def test_tiebreak_reversed_input_order():
    """Same graph, reversed component order → same f_block assignment."""
    rels = [
        Relationship(type=RelationType.DEPENDS_ON, from_id="COMP-B", to_id="COMP-C"),
        Relationship(type=RelationType.DEPENDS_ON, from_id="COMP-A", to_id="COMP-C"),
    ]
    ...
    m1 = auto_assign_f_blocks(_make_model(comps_order1, rels))
    m2 = auto_assign_f_blocks(_make_model(comps_order2, rels))

    fb1 = {c.id: c.f_block for c in m1.entities.components}
    fb2 = {c.id: c.f_block for c in m2.entities.components}
