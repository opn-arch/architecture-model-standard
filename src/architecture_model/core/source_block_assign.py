...
Bootstrap utility for models without source_block annotations.
"""
...
def auto_assign_source_blocks(
    model: ArchitectureModel,
    max_cluster_size: int = 5,
) -> ArchitectureModel:
    """Assign source_block values to components via dependency-graph clustering.
    ...
    """
    # Check if source_blocks already exist
    has_source_blocks = any(c.source_block for c in model.entities.components)
    ...
    assigned: dict[str, str] = {}  # comp_id → source_block_id
    cluster_id = 0
    ...
    for comp in sorted_comps:
        ...
        cluster_id += 1
        source_block = f"S{cluster_id}"
        cluster = [comp.id]
        assigned[comp.id] = source_block
        ...
        for neighbor in neighbors:
            ...
            assigned[neighbor] = source_block

    # Assign any remaining (no edges) components their own source_block
    for comp in comps:
        if comp.id not in assigned:
            cluster_id += 1
            assigned[comp.id] = f"S{cluster_id}"

    # Build new components with source_block assigned
    new_comps = []
    for comp in comps:
        new_comp = deepcopy(comp)
        new_comp.source_block = assigned[comp.id]
