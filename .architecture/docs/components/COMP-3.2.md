# Component: Graph & Analysis (COMP-3.2)

**Status:** Status.ACTIVE
**Description:** Import resolution, call graphs, interface extraction, behavior detection

## Files

| File | Functions | Classes |
|------|-----------|---------|
| `src/architecture_model/manifest/call_graph.py` | — | — |
| `src/architecture_model/manifest/interfaces.py` | — | — |
| `src/architecture_model/manifest/behavior.py` | — | — |
| `src/architecture_model/manifest/chains.py` | — | — |
| `src/architecture_model/manifest/test_analyzer.py` | — | — |

## Responsibilities

—

## Relationships

### Dependencies (outgoing)

| Target | Type | Description |
|--------|------|-------------|
| COMP-3.1 (Scanners) | depends-on | Graph analysis builds on scanner output |
| REQ-9 | satisfies | Graph analysis resolves imports to edges |

### Dependents (incoming)

| Source | Type | Description |
|--------|------|-------------|
| COMP-3.3 (Grouping & Generation) | depends-on | Grouping uses graph edges for affinity |
| COMP-3 (Manifest) | contains | — |

## Behaviors Realized

None

## Patterns

None

## Confidence

30%
