...
| `compute_function_readiness` | `sig: FunctionSignature, test_contracts: list[TestContract]` | `FunctionReadiness` | Compute readiness for a single function. |
| `compute_component_readiness` | `component: Component` | `ComponentReadiness` | Compute readiness for a single component. |
| `compute_regen_readiness` | `model: ArchitectureModel` | `RegenReadiness` | Compute full system regen readiness from enriched model. |
...
- **provides** `exposes_to_Authoring` → COMP-1 (Authoring) [Status, RelationType, ActorType, InterfaceType, ConstraintType, Priority, Strength, ComponentKind, BehaviorPattern, SymbolKind]
- **provides** `exposes_to_Pipeline` → COMP-2 (Pipeline) [load_block_model, load_model, validate_model_data, dump_model, save_model]
- **requires** `uses_Manifest` → COMP-3 (Manifest) [is_excluded_dir, collect_py_files, discover_source_files, discover_test_files]
- **requires** `uses_Schema` → COMP-7 (Schema) [load_profile, EnumExtension, EntityExtension, ConditionalRule, DomainProfile]
- **provides** `exposes_to_Extract` → COMP-6 (Extract) [Status, RelationType, ActorType, InterfaceType, ConstraintType, Priority, Strength, ComponentKind, BehaviorPattern, SymbolKind]
- **provides** `exposes_to_Manifest` → COMP-3 (Manifest) [Status, RelationType, ActorType, InterfaceType, ConstraintType, Priority, Strength, ComponentKind, BehaviorPattern, SymbolKind]
