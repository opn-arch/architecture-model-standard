...
- **provides** `exposes_to_Core` → COMP-1 (Core) [ModuleStatus, FunctionInfo, ClassInfo, ImportDetail, DecoratedFunction, ModuleInfo, InterfaceEdge, SubFunctionEntry, BlockManifest, MetricsResult]
- **provides** `exposes_to_Docs` → COMP-3 (Docs) [build_call_graph, trace_flow, map_flow_to_components, CallGraph, FlowTrace]
